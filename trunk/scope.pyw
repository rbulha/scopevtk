...
class VtkSpace:
    def __init__(self, parent):
      ...
      for i in range(800):
        ...
        self.values.InsertValue(i,i+10)
        self.PontosNaTela += 1 
      ...
      self.xyplot.GetProperty().SetPointSize(1)
    ...
    def UpdateDataDigitalPlot(self, data):
      ...
      self.polydata1.GetPointData().SetScalars(self.values)
      size = self.Parent.GetSize()
      self.renScope.SetScreenText(30,size.GetHeight()-20,timecount)
  ...
  def run(self):
    ...
    while self.IsRunning() and self.Waiting:
      ...
      if self.parent.USBXpress.WriteRaw(len(bufferwrite),bufferwrite) == 5:
        print 'PEDIU BUST'
      while self.parent.USBXpress.CheckRXQueue() == 0:
          sleep(0.1)
...
class ScopeFrm(wx.Frame):
    def __init__(self, parent):
      ...
      self.Bind(EVT_UPDATE_DIGITAL_PLOT, self.OnUpdateDigitalPlotArea)
    ...
    def OnUpdateDigitalPlotArea(self, event):      
      ...
      if bytestoread > 0:
        ...
        self.VtkSpace1.UpdateDataDigitalPlot(integerdata)     
    ...
    def OnUpdatePlotArea(self, event):
      ...
      if self.USBXpress.WriteRaw(len(bufferwrite),bufferwrite) == 5:
        ...
        media = sum(integerdata) / len(integerdata)
